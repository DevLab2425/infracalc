declare interface Window { InfraCalc: any }
...
window.InfraCalc = (window.InfraCalc || {
  ...
  init: () => {
    ...
    const { data, calculateResourceRate, calculateBuildingRequirements, setupToggleButtons } = window.InfraCalc;
    
    const resourceSelector: any = document.getElementById("resource-selector");
    const buildings: any = document.getElementById("building");
  ...
  toggleMode: (ev: { target: HTMLInputElement }) => {
    const { target : { value } } = ev;
    
    console.debug('InfraCalc::Toggle Mode ->', value);

    const calcs = document.querySelectorAll('.calculator');
    const activeRadio = document.getElementById(`${value}-calculation`) as HTMLInputElement;
  ...
  calculateBuildingRequirements: (ev: SubmitEvent) => {
    ...
    const buildingAmountField = document.getElementById("building-amount") as HTMLInputElement
    
    const itemKey = Object.keys(data)[buildingSelector.selectedIndex];
    const item = data[itemKey];
    const amount = Number(buildingAmountField.value);
    const table = document.getElementById("calculator-results");
    const messageElement = document.getElementById("results-message");
  
    const plural = (amount > 1) && "s";
    const message = `Requirements to satisfy ${amount} ${item.building} building${plural}`
    ...
    itemCosts(itemKey, results, amount);
  ...
  calculateResourceRate: (ev: SubmitEvent) => {
    ...
    const amountField = document.getElementById("rate-amount") as HTMLInputElement;
    const resultsTable = document.getElementById("calculator-results") as HTMLTableElement;
    ...
    const item = Object.keys(data)[resourceSelector.selectedIndex];
    const amount = Number(amountField.value);
  
    if (!validateAmount(amount, messageElement, resultsTable, "Required buildings for " + item)) {
      return;
    }
  
    const requiredRate: number = amount / time;
    const results = {};
  
    itemCosts(item, results, requiredRate / data[item].rate);
    populateResults(item, resultsTable, results);
    displayResults();
  ...
  populateResults: (item: string, table: any, results: any) => {
    const { data, sumRequired, displayResults } = window.InfraCalc;
